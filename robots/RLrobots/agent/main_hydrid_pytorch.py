...
import json
import numpy as np
from hybrid_cnn_dqn_agent_pytorch import HybridCNNDQNAgent, normalize_state
import traceback
from PIL import Image

# Initialize PyTorch Hybrid CNN DQN agent for grayscale image + state vector input
IMAGE_SIZE = 84
agent = HybridCNNDQNAgent(image_channels=4, image_size=84, state_size=13, action_size=8, learning_rate=0.0015, epsilon_max=1)
agent.load_model(learning_rate=0.0015, filename='hybrid_cnn_dqn_model_pytorch.pth')  # Load existing model if available
...
parser = argparse.ArgumentParser(description='Robot Hybrid CNN DQN Training with Grayscale Image + State Vector Input')
...
async def handle_robot(websocket):
    global current_state_image, current_state_vector, previous_state_image, previous_state_vector, previous_action, episode_reward, episode_count, total_reward, training_losses, step_count, win_episode, death_episode, play_time, no_learning, optimal_actions, frame_buffer
    
    try:
        print("Hybrid CNN Grayscale Robot connected")
        async for message in websocket:
            try:
                ...
                # Extract and normalize state vector from JSON data
                state_vector = normalize_state(state_data)
                ...
                if args.play == True or no_learning == True:
                    action = agent.get_optimal_action(image_state, state_vector)
                    await websocket.send(str(action))
                    continue
                
                # Get action from Hybrid CNN DQN agent
                action = agent.act(image_state, state_vector)
        ...
        print("Hybrid CNN Grayscale Client disconnected. Final cleanup")
        # End episode
        if args.play == True or no_learning == True:
            return
        ...
        if previous_state_image is not None and previous_action is not None:            
            ...
            print(f"Hybrid CNN Grayscale Episode {episode_count} ended with reward: {episode_reward:.2f}, play_time: {play_time}")
            print(f"Hybrid CNN Grayscale Optimal actions: {optimal_action_counts}")
...
def save_training_stats():
    ...
    with open('hybrid_cnn_grayscale_training_stats_pytorch.json', 'w') as f:
        json.dump(training_stats, f, indent=2)
    print("Hybrid CNN Grayscale Training stats saved")

async def main():
    server = await websockets.serve(handle_robot, "localhost", 5000)
    print("PyTorch Hybrid CNN Grayscale DQN Robot server started on ws://localhost:5000")
    print(f"Hybrid CNN Grayscale Agent epsilon: {agent.epsilon}")
    print(f"Hybrid CNN Grayscale Using device: {agent.device}")
    print(f"Hybrid CNN Grayscale Input channels: {agent.image_channels}")
    print(f"Hybrid CNN Grayscale State size: {agent.state_size}")
